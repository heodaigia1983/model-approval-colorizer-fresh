...
const DEFAULT_BASE_COLOR = "#B8860B";
...
function getBaseColor() {
  const text = document.getElementById("baseColorText");
  const picker = document.getElementById("baseColorPicker");

  const fromText = String(text?.value || "").trim();
  if (isValidHexColor(fromText)) return fromText;

  const fromPicker = String(picker?.value || "").trim();
  if (isValidHexColor(fromPicker)) return fromPicker;
...
document.getElementById("baseColorText").addEventListener("change", (e) => {
  const color = String(e.target.value || "").trim();
  if (isValidHexColor(color)) {
    syncColorInputs(color);
  } else {
    syncColorInputs(DEFAULT_BASE_COLOR);
  }
});
